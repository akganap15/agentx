...
export default function App() {
  ...
  return (
    <div className="app">
      ...
      <Header business={business} onCall={() => setCallActive(true)} theme={theme} onToggleTheme={toggleTheme} />
...
function LoginScreen({ onLogin, onSetup }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (status === 401) {
        // No account found — launch setup wizard pre-filled with this email
        onSetup(email.trim())
      } else {
...
function Header({ business, onCall, theme, onToggleTheme }) {
  return (
